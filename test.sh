...
echo "--- Task Parser ---"
obsidian eval 'var tp=require("/path/to/repo/src/task-parser.js"); var r=tp.parseTaskLine("- [ ] Write spec @bucket:deep-work",{path:"t.md"},0); r&&r.bucket==="deep-work" ? "PASS: bucket parsing" : "FAIL: bucket parsing"'
obsidian eval 'var tp=require("/path/to/repo/src/task-parser.js"); var r=tp.parseTaskLine("- [ ] Task @1.5h",{path:"t.md"},0); r&&r.durationMinutes===90 ? "PASS: duration parsing 1.5h=90m" : "FAIL: duration="+(r?r.durationMinutes:"null")'
obsidian eval 'var tp=require("/path/to/repo/src/task-parser.js"); var r=tp.parseTaskLine("- [ ] Task @30m",{path:"t.md"},0); r&&r.durationMinutes===30 ? "PASS: duration parsing 30m" : "FAIL: duration="+(r?r.durationMinutes:"null")'

# Test 7: Filter engine
echo "--- Filter Engine ---"
obsidian eval 'var fe=require("/path/to/repo/src/filter-engine.js"); fe.evaluateFilter({field:"bucket",op:"eq",value:"deep-work"},{bucket:"deep-work"})===true ? "PASS: filter eq" : "FAIL: filter eq"'
obsidian eval 'var fe=require("/path/to/repo/src/filter-engine.js"); !fe.evaluateFilter({field:"bucket",op:"eq",value:"other"},{bucket:"deep-work"}) ? "PASS: filter neq" : "FAIL: filter neq"'
obsidian eval 'var fe=require("/path/to/repo/src/filter-engine.js"); fe.evaluateFilter({op:"and",filters:[{field:"bucket",op:"eq",value:"x"},{field:"bucket",op:"eq",value:"y"}]},{bucket:"x"})===false ? "PASS: filter and false" : "FAIL: filter and"'

# Test 8: Budget state
echo "--- Budget State ---"
obsidian eval 'var bs=require("/path/to/repo/src/budget-state.js"); bs.getBudgetState(0,10).state==="normal" ? "PASS: budget normal" : "FAIL: budget normal"'
obsidian eval 'var bs=require("/path/to/repo/src/budget-state.js"); bs.getBudgetState(8,10).state==="warning" ? "PASS: budget warning" : "FAIL: budget warning"'
obsidian eval 'var bs=require("/path/to/repo/src/budget-state.js"); bs.getBudgetState(10,10).state==="over" ? "PASS: budget over" : "FAIL: budget over"'
